...
workdir  := "C:\ZZZ-OD"
odWinTitle := "绝区零 一条龙 ahk_class Qt680QWindowIcon"

logFile  := "C:\ZZZ-OD\ZZZ-autorun\logs\closedloop.log"

if (!A_IsAdmin) {
    FormatTime, ts,, yyyy-MM-dd HH:mm:ss
    FileAppend, %ts%  BOOT: not admin, requesting elevation for AutoHotkey`r`n, %logFile%
    Run, *RunAs "%A_AhkPath%" "%A_ScriptFullPath%", %A_ScriptDir%, UseErrorLevel
    if ErrorLevel
        FileAppend, %ts%  ERROR: elevation request failed or was cancelled`r`n, %logFile%
    ExitApp
}
...
; OCR 定位「启动一条龙 / 开始」按钮，默认不再按固定像素误点
startOcrScript   := "C:\ZZZ-OD\ZZZ-autorun\scripts\ocr_find_text.py"
startOcrInterval := 1500
fallbackFixedClick := false

; 紧急兜底坐标，只有 fallbackFixedClick := true 时才会使用
; 注意：必须用 Window Spy 里的 Client 坐标，不是 Screen 坐标
btnX := 1470
btnY := 1055

; 检测到弹窗后，等待多久再重启（毫秒）
; 10800000 = 3 小时 | 7200000 = 2 小时 | 3600000 = 1 小时 | 测试用可改成 60000（1 分钟）
restartDelay  := 10800000
...
LogRunPythonError(msg) {
    static lastErrorLog := 0
    if (lastErrorLog = 0 || A_TickCount - lastErrorLog >= 60000) {
        lastErrorLog := A_TickCount
        Log(msg)
    }
}

RunPythonHidden(scriptPath, resultFile, extraArgs := "") {
    global ocrPython

    cmd := """" . ocrPython . """ """ . scriptPath . """ """ . resultFile . """"
    if (extraArgs != "")
        cmd .= " " . extraArgs

    try {
        RunWait, %cmd%, %A_ScriptDir%, Hide UseErrorLevel
        if (ErrorLevel = "ERROR") {
            LogRunPythonError("OCR: failed to launch python cmd=" . cmd)
            return false
        }
        if (ErrorLevel != 0)
            LogRunPythonError("OCR: python exited with code=" . ErrorLevel . " cmd=" . cmd)
        return true
    } catch e {
        LogRunPythonError("OCR: RunWait exception message=" . e.Message . " what=" . e.What . " extra=" . e.Extra)
        return false
    }
}
...
CloseOneDragon_Force() {
    ...
    if WinExist(odWinTitle) {
        WinGet, hwnd, ID, %odWinTitle%
        WinGet, pid, PID, ahk_id %hwnd%
        if (pid) {
            Process, Close, %pid%
            Log("CLOSE: Process.Close pid=" . pid)
            WinWaitClose, ahk_id %hwnd%, , 10
            if ErrorLevel
                Log("CLOSE: Qt window still present after close timeout")
        } else {
...
; --- Emergency fixed-coordinate fallback, disabled by default ---
FixedClickStartButton(hwnd, reason) {
    global btnX, btnY, startClickTime
    ...
    Log("START: fallback fixed click reason=" . reason . " x=" . sx . " y=" . sy)
    startClickTime := A_TickCount
    return true
}

; --- OCR click helper; returns true only after a real click ---
ClickStartButtonByOcr(hwnd, timeoutMs) {
    global ocrPython, startOcrScript, startOcrInterval, startClickTime
    ...
    Loop
    {
        startOcrResultFile := A_Temp . "\zzz_start_ocr_result.txt"
        FileDelete, %startOcrResultFile%
        if (!RunPythonHidden(startOcrScript, startOcrResultFile, hwnd)) {
            startOcrResult := "ERROR|0|0|python launch failed"
        } else {
            FileEncoding, UTF-8
            FileRead, startOcrResult, %startOcrResultFile%
            FileEncoding, CP0
            FileDelete, %startOcrResultFile%
        }

        startOcrResult := Trim(startOcrResult)
        if (startOcrResult = "")
            startOcrResult := "ERROR|0|0|empty result"

        parts := StrSplit(startOcrResult, "|")
        status := parts[1]
        ...
        if (status = "FOUND") {
            ...
            if (sx > 0 && sy > 0) {
                ...
                Log("START: OCR clicked start button text=" . matchedText . " x=" . sx . " y=" . sy)
                startClickTime := A_TickCount
                return true
            }

            Log("START OCR: invalid coordinate result=" . startOcrResult)
        } else if (startOcrResult != lastStartOcrResult) {
            lastStartOcrResult := startOcrResult
            Log("START OCR: " . startOcrResult)
        }

        if (A_TickCount >= deadline)
            break

        Sleep, %startOcrInterval%
    }

    return false
}

; --- Start OneDragon and click the Start button via OCR ---
StartAndClick() {
    global launcher, workdir, odWinTitle, startupWaitSec, postLaunchSleep
    global ocrPython, startOcrScript, startOcrInterval, fallbackFixedClick
    global startClickTime

    existingHwnd := WinExist(odWinTitle)
    if (existingHwnd) {
        WinGet, existingPid, PID, ahk_id %existingHwnd%
        Log("START: existing OneDragon window found hwnd=" . existingHwnd . " pid=" . existingPid)
        if (ClickStartButtonByOcr(existingHwnd, 10000))
            return true
        Log("START: existing OneDragon window has no start button, monitor only")
        startClickTime := A_TickCount
        return true
    }
    ...
    WinWait, %odWinTitle%, , %startupWaitSec%
    if ErrorLevel {
        Log("START: timeout waiting Qt main window")
        return false
    }

    WinActivate, %odWinTitle%
    WinWaitActive, %odWinTitle%, , 10
    ...
    if (ClickStartButtonByOcr(hwnd, startupWaitSec * 1000))
        return true

    if (fallbackFixedClick) {
        Log("START: OCR timeout, using fixed-coordinate fallback")
        return FixedClickStartButton(hwnd, "ocr_timeout")
    }

    Log("START: OCR start button not found before timeout")
    return false
}
...
Log("BOOT: closed-loop started. admin=" . A_IsAdmin . " ocr_interval=" . ocrInterval . "ms fail_check=" . failCheckInterval . "ms")
...
Loop
{
    ...
    Loop
    {
        ...
        if (nowTick - lastOcrCheck >= ocrInterval) {
            ...
            if (!RunPythonHidden(ocrScript, ocrResultFile)) {
                ocrResult := "ERROR"
            } else {
