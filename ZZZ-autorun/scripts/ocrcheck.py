...
TOP_OUT_KEYWORDS = ("其他地方登录",)
NETWORK_RETRY_KEYWORDS = ("网络错误", "稍后重试")
SERVER_TIMEOUT_KEYWORDS = ("服务器连接超时", "继续尝试连接")
DROP_CHARS = " \t\r\n，,。.！!：:；;？?（）()[]【】"
...
def normalize_ocr_text(text: str) -> str:
    return text.translate(str.maketrans("", "", DROP_CHARS))


def is_top_out_text(text: str) -> bool:
    normalized = normalize_ocr_text(text)
    if any(keyword in normalized for keyword in TOP_OUT_KEYWORDS):
        return True
    if any(keyword in normalized for keyword in NETWORK_RETRY_KEYWORDS):
        return True
    return any(keyword in normalized for keyword in SERVER_TIMEOUT_KEYWORDS)
...
async def main() -> str:
    ...
    return "FOUND" if is_top_out_text(text) else "NOT_FOUND"
